...
| **3 · Genome & genes** | Consensus completeness, the genome landscape (missing calls, **deletions**, SNPs per bin and **per callable kb**, het, indels), the **deletions** found by comparing each sample's stretches without reads with the rest of the cohort, functional impact (snpEff), gene burden, variable genes and dN/dS. A cohort mapped against several references is shown one reference at a time, each with its own samples, length, genes and masked regions. |
...
| `--ref-gff REF=<genes.gff3> …` | The same, **per reference**: each reference's own genes under its own samples, where the cohort was mapped against several. With `--ref-mask` and `--ref-fai`, it replaces `--gff` and `--mask-bed` (the pipeline passes these) |
| `--ref-mask REF=<mask.bed> …` | Each reference's masked regions (the mask-region toggle) |
| `--ref-fai REF=<ref.fa.fai> …` | Each reference's contigs, in order: genes and masked regions of a multi-contig reference are placed as its samples' profiles are binned, contigs end to end |
