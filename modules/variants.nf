...
process CALL_FREEBAYES {
    ...
    # Hom / het are defined in ONE place (bin/vcf_filter_rules.py) so the two FreeBayes paths
    # cannot drift apart. Run it with --help to see the expression it builds.
    hom_rule=\$(vcf_filter_rules.py hom --min-dp !{params.filter_min_dp} --min-alt-fwd !{params.min_alt_fwd} --min-alt-rev !{params.min_alt_rev} \
        --hom-threshold !{params.hom_threshold} --het-min-frac !{params.het_min_frac})
    het_rule=\$(vcf_filter_rules.py het --min-dp !{params.filter_min_dp} --min-alt-fwd !{params.min_alt_fwd} --min-alt-rev !{params.min_alt_rev} \
        --hom-threshold !{params.hom_threshold} --het-min-frac !{params.het_min_frac})
    ...
    awk -v HETMIN=!{params.het_min_frac} \
        -f !{projectDir}/bin/format_snps_for_backbone.awk valid_snps.vcf | bgzip -@ !{task.cpus} -c > valid_snps_formatted.vcf.gz
